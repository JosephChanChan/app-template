								...
								<select name="status" class="form-control">
									<c:if test="${statusList != null }">
									   ...
									   <c:forEach var="dataDictionary" items="${statusList}">
									   		<option <c:if test="${dataDictionary.valueId == param.status }">selected="selected"</c:if>
					...
					<a href="${ctx}/appinfo/add.html" class="btn btn-success btn-sm">新增APP基础信息</a>
                      ...
                      <ul class="dropdown-menu" role="menu">
                        <li>
                        	...
                        	<c:choose>
											...
											<c:when test="${appInfo.status == 4}">
												<a class="saleSwichOpen" saleSwitch="close" appinfoid=${appInfo.id }  appsoftwarename=${appInfo.softwareName } data-toggle="tooltip" data-placement="top" title="" data-original-title="您可以点击下架来停止发布您的APP，市场将不提供APP的下载">下架</a>
											</c:when>
										</c:choose>
                        </li>
                        <li>
                        ...
                        <li><a class="modifyVersion" 
											...
											statusname="${appInfo.statusName }"	 versionNo="${appInfo.versionNo }"
