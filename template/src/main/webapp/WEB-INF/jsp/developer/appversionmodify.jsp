    ...
    <div class="x_panel">
	  ...
	  <p class="text-muted font-13 m-b-30"></p>
						...
						<tbody>
							<c:forEach var="appVersion" items="${appVersionList }" varStatus="status">
								<tr role="row" class="odd">
									...
									<td>
									<a href="${ctx}/appinfo/download?filename=${appVersion.apkFileName }">${appVersion.apkFileName }</a>
									</td>
        ...
        <div class="x_content" style="display: block;">
         <br>
        <form class="form-horizontal form-label-left" action="${ctx}/appversion/versionmodify.do" method="post" enctype="multipart/form-data">
           ...
           <div class="item form-group">
			...
			${msg }
               ...
               <script type="text/javascript">
                   $(function () {
                      /* 判断applocpath是否为空 不为空就显示下载地址 和删除
                       为空就显示上传*/
                      var obj=$("#apkLocPath");
                      var appLocPath=obj.val();
                      var apkFileName=$("#apkFileName");
                      if(appLocPath!=null&&appLocPath!=''){
                          //文件下载连接
                          $("#apkFile").html("<p>"+apkFileName.val()+"<span><a href='${ctx}/appinfo/download?filename="+apkFileName.val()+"'>下载</a></span><span><a href=\"javascript:;\" onclick=\"deletefile()\" >删除</a></span></p>")
                      }else {
                          $("#uploadfile").show();
                      }
                   })
                   ...
                   /*  如果logoFile 不等于空 或者 uploadfile有文件* 则保存 否则不能保存*/
                   function sum() {
                       var content=$("#apkFile").text();
                       var file=$("#attach").val();

                       if(content !=null && content.length !=0 &&content !='' || file !=''){
          ...
          <div class="form-group">
            <div class="col-md-6 col-md-offset-3">
              <button id="send" type="button" class="btn btn-success" onclick="sum()">保存</button>
