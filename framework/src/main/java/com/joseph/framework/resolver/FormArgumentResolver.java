package com.joseph.framework.resolver;

import com.joseph.framework.exception.ApplicationException;
import com.joseph.framework.form.Form;
import com.joseph.framework.result.Result;
import com.joseph.framework.utils.stomp.ReflectionUtils;
	...
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest,
		...
		if(contentType != null && contentType.startsWith(MediaType.APPLICATION_JSON_VALUE)){
			try{
				...
				throw new ApplicationException(Result.INVALID_PARAM, "参数格式不符合：" + MediaType.APPLICATION_JSON_VALUE);
			}
		}
		if(value == null){
		...
		if(br.getFieldErrorCount() > 0){
			...
			if(br.getFieldError().isBindingFailure() ){
				...
				errorMsg = br.getFieldError().getDefaultMessage();
			}
